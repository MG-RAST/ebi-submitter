...
"\n\ncreate_xml.pl >>> create the ENA XML file for an MG-RAST project and submit it to EBI\n";
    print
"create_xml.pl -user <username> -password <password> -project_id <project id>\n";
    print "\nOPTIONS\n";
...
unless ($submission_id) {
    print STDERR
      "Can't create submission, no submisison ID or project provided\n";
    exit;
}
...
my $ftp = Net::FTP->new($ftp_ena)
  or die "Cannot connect to $ftp_ena: $@";    # , Debug => 1
$ftp->login( $user, $password )
  or die "Cannot login using $user and $password", $ftp->message;
$ftp->mkdir($project_id);
$ftp->cwd($project_id);

# create project XML
my ( $project_data, $error ) =
  get_json_from_url( $ua, $url, $resource, $project_id, $options );
...
#create samples xml
  ...
  # create samples XML
  >> >> >> > 80463312 fc5836fac5aef9994ecb5811b8e81771 my $sample_xml = <<"EOF";
...
foreach my $sample_obj ( @{ $project_data->{samples} } ) {
    ...
    my ( $sample_data, $err ) =
      get_json_from_url( $ua, $url, $sample_resource, $metagenome_sample_id,
        $options );
    if ($error) {
        print STDERR
"Error retrieving sample $metagenome_sample_id with error message $err!\n";
        next;
    }
...
foreach my $library_obj ( @{ $project_data->{metagenomes} } ) {
    ...
    my ( $experiment_data, $err ) =
      get_json_from_url( $ua, $url, $experiment_resource, $metagenome_id,
        $options );
    if ($error) {
        print STDERR
          "Error retrieving library data for $metagenome_id (ERROR:$err)\n";
        next;
    }
    $experiment_xml .=
      get_experiment_xml( $experiment_data, $center_name, $study_ref_name );
}
...
foreach my $metagenome_obj ( @{ $project_data->{metagenomes} } ) {
    ...
    my ( $file_name, $md5, $err ) =
      &prep_files_for_upload( $ftp, $url, $stage_name, $metagenome_id );
    if ($err) {
    ...
    my ( $run_data, $err2 ) =
      get_json_from_url( $ua, $url, $run_resource, $metagenome_id, '' );

    $run_xml .=
      get_run_xml( $run_data, $center_name, $metagenome_id, $file_name, $md5,
        $project_id );
}
...
if ($submit) {
    ...
    print "Submitting\n" if ($verbose);
    submit( $study_xml, $sample_xml, $experiment_xml, $run_xml, $submission_id,
        $center_name, $files );
}
else {
    print $study_xml . "\n";
...
sub get_json_from_url {
    ...
    my $response =
      $ua->get( join "/", $url, $resource, $metagenome_id, $options );
    ...
    eval { $data = $json->decode( $response->content ) };
...
sub get_sample_xml {
    ...
    foreach my $metadata_key ( keys %{ $data->{env_package}->{metadata} } ) {
        if ( exists( $sample_attribute_table->{$metadata_key} ) ) {
            ...
            $sample_attribute_table->{$metadata_key} =
              $data->{env_package}->{metadata}->{$metadata_key};
        }
...
sub get_run_xml {
    my ( $data, $center_name, $metagenome_id, $filename, $file_md5,
        $project_id ) = @_;
    my $run_id   = $data->{id};
...
sub submit {
    ...
    my ( $study_xml, $sample_xml, $experiment_xml, $run_xml, $submission_id,
        $center_name, $files )
      = @_;
...
EOF
    ...
    my $cmd =
"curl -k -F \"SUBMISSION=\@submission.xml\" -F \"STUDY=\@study.xml\" -F \"SAMPLE=\@sample.xml\" -F \"EXPERIMENT=\@experiment.xml\" -F \"RUN=\@run.xml\" \"$ena_url\"";
    print "$cmd\n";
    my $receipt = `$cmd`;
...
sub prep_files_for_upload {
    ...
    unless ( $response->is_success ) {
        print STDERR "Error retrieving data for "
          . ( join "/", $url, $resource, $metagenome_id, "\n" );
...
sub decode {
    ...
    eval { $data = $json->decode($json_string); };
